...
namespace VoyageurDeCommerce.modele.parseur
{
    ...
    public class Parseur
    {
        ...
        public Parseur(String nomDuFichier)
        {
            ...
            this.adresseFichier = Path.GetDirectoryName(Assembly.GetEntryAssembly().Location) + @"\ressource\" + nomDuFichier;
        ...
        public void Parser()
        {
            ...
            using (StreamReader stream = new StreamReader(this.adresseFichier))
            {
                ...
                while ((ligne = stream.ReadLine()) != null)
                {
                    ...
                    if (morceaux[0] == "ROUTE")
                    {
                        ListeRoutes.Add(MonteurRoute.Creer(morceaux, this.listeLieux));
                    }
                    else
                    {
